...
from workflows.acquisition.proximity_dataset_writer import ProximityDatasetWriter
...
class ProximityAcquisitionWindow(QtWidgets.QWidget):
    ...
    def __init__(self, sensor, sensor_cfg: Dict):
        ...
        self.dataset_writer: Optional[ProximityDatasetWriter] = None
    ...
    def create_csv_file(self):
        if self.dataset_writer:
            self.dataset_writer.close()
        self.dataset_writer = ProximityDatasetWriter.create_new(DEFAULT_LOG_DIR)
        self.selected_file_path.setText(str(self.dataset_writer.path))
        self.sample_num.setText("0")
        self._append_log(f"已创建数据文件：{self.dataset_writer.path}")
    ...
    def select_csv_file(self):
        ...
        try:
            existing_pairs = ProximityDatasetWriter.read_existing_pitch_roll_pairs(file_path)
        except Exception as exc:
            existing_pairs = set()
        ...
        if self.dataset_writer:
            self.dataset_writer.close()
        self.dataset_writer = ProximityDatasetWriter.open_existing(file_path)
        self._append_log(f"已选择数据文件：{file_path}")

    def save_data_row(self):
        if not self.collecting or self.dataset_writer is None or self.current_frame is None:
            return
        if not self.data_recording_ready or not self.moving_forward or self.sampled_once:
            return
